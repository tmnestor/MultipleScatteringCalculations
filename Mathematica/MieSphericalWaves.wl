...
(* ---------------------------------------------------------------
   HARVESTED FROM THE RESEARCH ARCHIVE
   MyRecursiveHarmonicDerivatives.nb, in
   ~/Documents/WaveTheory-Research/MathematicaSourceCode/GreensTensorCalculations

   Emitted programmatically by Mathematica/emitVerifiedDefs.wl.
   DO NOT EDIT BY HAND -- regenerate instead, so that the source of
   truth stays the archive plus the verification, not this copy.
   --------------------------------------------------------------- *)

(* archive definition 1: Tn *)
Tn[i_, u_, norm_List] :=
  λ*norm[[i]]*Sum[u[k, k], {k, 3}] + μ*Sum[norm[[j]]*(u[i, j] + u[j, i]), {j, 3}];

(* archive definition 2: r *)
r[x_, y_, z_] :=
  Sqrt[x^2 + y^2 + z^2];

(* archive definition 3: θ *)
θ[x_, y_, z_] :=
  ArcTan[z, Sqrt[x^2 + y^2]];

(* archive definition 4: ϕ *)
ϕ[x_, y_, z_] :=
  ArcTan[x, y];

(* archive definition 5: ap *)
ap[n_, m_] :=
  If[Element[n, Integers] && n >= 0 && Element[m, Integers] && Abs[m] <= n, -Sqrt[((n + m + 1)*(n - m + 1))/((2*n + 1)*(2*n + 3))], 0];

(* archive definition 6: am *)
am[n_, m_] :=
  If[Element[n, Integers] && n >= 0 && Element[m, Integers] && Abs[m] <= n, Sqrt[((n + m)*(n - m))/((2*n - 1)*(2*n + 1))], 0];

(* archive definition 7: bp *)
bp[n_, m_] :=
  If[Element[n, Integers] && n >= 0 && Element[m, Integers] && Abs[m] <= n, Sqrt[((n + m + 2)*(n + m + 1))/((2*n + 1)*(2*n + 3))], 0];

(* archive definition 8: bm *)
bm[n_, m_] :=
  If[Element[n, Integers] && n >= 0 && Element[m, Integers] && Abs[m] <= n, Sqrt[((n - m)*(n - m - 1))/((2*n - 1)*(2*n + 1))], 0];

(* archive definition 9: J *)
J[K_, n_, m_, r_, θ_, ϕ_] :=
  SphericalBesselJ[n, K*r]*SphericalHarmonicY[n, m, θ, ϕ];

(* archive definition 53: Y *)
Y[n_, m_, x_, y_, z_] :=
  (Sqrt[((2*n + 1)*(n + m)!*(n - m)!)/(4*Pi)]*Sum[(1/(i!*j!*k!*2^i*2^j))*(KroneckerDelta[i + j + k, n]*KroneckerDelta[i - j, m]*(x - I*y)^j*(-x - I*y)^i*z^k), {i, 0, n}, {j, 0, n}, {k, 0, n}])/Sqrt[x^2 + y^2 + z^2]^n;

(* The Cartesian derivative operators.  Each index below is the
   variant that verifyDerivativeOperators.wl measured against direct
   differentiation; all variants in the archive agreed, to 1e-29. *)

(* d_1  <- archive definition 10, verified = D[.., x_1] *)
Subscript[d, 1][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Simplify[PowerExpand[(K/2)*(bm[n, m]*F[K, n - 1, m + 1, r, θ, ϕ] + bp[n, m]*F[K, n + 1, m + 1, r, θ, ϕ] - bm[n, -m]*F[K, n - 1, m - 1, r, θ, ϕ] - bp[n, -m]*F[K, n + 1, m - 1, r, θ, ϕ])]];

(* d_2  <- archive definition 11, verified = D[.., x_2] *)
Subscript[d, 2][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Simplify[PowerExpand[(-I)*(K/2)*(bm[n, m]*F[K, n - 1, m + 1, r, θ, ϕ] + bp[n, m]*F[K, n + 1, m + 1, r, θ, ϕ] + bm[n, -m]*F[K, n - 1, m - 1, r, θ, ϕ] + bp[n, -m]*F[K, n + 1, m - 1, r, θ, ϕ])]];

(* d_3  <- archive definition 12, verified = D[.., x_3] *)
Subscript[d, 3][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Simplify[PowerExpand[K*(am[n, m]*F[K, n - 1, m, r, θ, ϕ] + ap[n, m]*F[K, n + 1, m, r, θ, ϕ])]];

(* d_1, 1  <- archive definition 13, verified = D[.., x_1] D[.., x_1] *)
Subscript[d, 1, 1][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  (1/4)*K^2*(bm[n, -m]*(bm[-1 + n, 1 - m]*F[K, -2 + n, -2 + m, r, θ, ϕ] - bm[-1 + n, -1 + m]*F[K, -2 + n, m, r, θ, ϕ] + bp[-1 + n, 1 - m]*F[K, n, -2 + m, r, θ, ϕ] - bp[-1 + n, -1 + m]*F[K, n, m, r, θ, ϕ]) + bm[n, m]*((-bm[-1 + n, -1 - m])*F[K, -2 + n, m, r, θ, ϕ] + bm[-1 + n, 1 + m]*F[K, -2 + n, 2 + m, r, θ, ϕ] - bp[-1 + n, -1 - m]*F[K, n, m, r, θ, ϕ] + bp[-1 + n, 1 + m]*F[K, n, 2 + m, r, θ, ϕ]) + bp[n, -m]*(bm[1 + n, 1 - m]*F[K, n, -2 + m, r, θ, ϕ] - bm[1 + n, -1 + m]*F[K, n, m, r, θ, ϕ] + bp[1 + n, 1 - m]*F[K, 2 + n, -2 + m, r, θ, ϕ] - bp[1 + n, -1 + m]*F[K, 2 + n, m, r, θ, ϕ]) + bp[n, m]*((-bm[1 + n, -1 - m])*F[K, n, m, r, θ, ϕ] + bm[1 + n, 1 + m]*F[K, n, 2 + m, r, θ, ϕ] - bp[1 + n, -1 - m]*F[K, 2 + n, m, r, θ, ϕ] + bp[1 + n, 1 + m]*F[K, 2 + n, 2 + m, r, θ, ϕ]));

(* d_1, 2  <- archive definition 14, verified = D[.., x_1] D[.., x_2] *)
Subscript[d, 1, 2][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  (1/4)*I*K^2*(bm[n, -m]*(bm[-1 + n, 1 - m]*F[K, -2 + n, -2 + m, r, θ, ϕ] + bm[-1 + n, -1 + m]*F[K, -2 + n, m, r, θ, ϕ] + bp[-1 + n, 1 - m]*F[K, n, -2 + m, r, θ, ϕ] + bp[-1 + n, -1 + m]*F[K, n, m, r, θ, ϕ]) - bm[n, m]*(bm[-1 + n, -1 - m]*F[K, -2 + n, m, r, θ, ϕ] + bm[-1 + n, 1 + m]*F[K, -2 + n, 2 + m, r, θ, ϕ] + bp[-1 + n, -1 - m]*F[K, n, m, r, θ, ϕ] + bp[-1 + n, 1 + m]*F[K, n, 2 + m, r, θ, ϕ]) + bp[n, -m]*(bm[1 + n, 1 - m]*F[K, n, -2 + m, r, θ, ϕ] + bm[1 + n, -1 + m]*F[K, n, m, r, θ, ϕ] + bp[1 + n, 1 - m]*F[K, 2 + n, -2 + m, r, θ, ϕ] + bp[1 + n, -1 + m]*F[K, 2 + n, m, r, θ, ϕ]) - bp[n, m]*(bm[1 + n, -1 - m]*F[K, n, m, r, θ, ϕ] + bm[1 + n, 1 + m]*F[K, n, 2 + m, r, θ, ϕ] + bp[1 + n, -1 - m]*F[K, 2 + n, m, r, θ, ϕ] + bp[1 + n, 1 + m]*F[K, 2 + n, 2 + m, r, θ, ϕ]));

(* d_2, 1  <- archive definition 15, verified = D[.., x_2] D[.., x_1] *)
Subscript[d, 2, 1][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Subscript[d, 1, 2][K, n, m, r, θ, ϕ][F];

(* d_1, 3  <- archive definition 17, verified = D[.., x_1] D[.., x_3] *)
Subscript[d, 1, 3][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  (1/2)*K^2*((-bm[n, -m])*(am[-1 + n, -1 + m]*F[K, -2 + n, -1 + m, r, θ, ϕ] + ap[-1 + n, -1 + m]*F[K, n, -1 + m, r, θ, ϕ]) + bm[n, m]*(am[-1 + n, 1 + m]*F[K, -2 + n, 1 + m, r, θ, ϕ] + ap[-1 + n, 1 + m]*F[K, n, 1 + m, r, θ, ϕ]) - bp[n, -m]*(am[1 + n, -1 + m]*F[K, n, -1 + m, r, θ, ϕ] + ap[1 + n, -1 + m]*F[K, 2 + n, -1 + m, r, θ, ϕ]) + bp[n, m]*(am[1 + n, 1 + m]*F[K, n, 1 + m, r, θ, ϕ] + ap[1 + n, 1 + m]*F[K, 2 + n, 1 + m, r, θ, ϕ]));

(* d_3, 1  <- archive definition 18, verified = D[.., x_3] D[.., x_1] *)
Subscript[d, 3, 1][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Subscript[d, 1, 3][K, n, m, r, θ, ϕ][F];

(* d_2, 2  <- archive definition 19, verified = D[.., x_2] D[.., x_2] *)
Subscript[d, 2, 2][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  (-(1/4))*K^2*(bm[n, -m]*(bm[-1 + n, 1 - m]*F[K, -2 + n, -2 + m, r, θ, ϕ] + bm[-1 + n, -1 + m]*F[K, -2 + n, m, r, θ, ϕ] + bp[-1 + n, 1 - m]*F[K, n, -2 + m, r, θ, ϕ] + bp[-1 + n, -1 + m]*F[K, n, m, r, θ, ϕ]) + bm[n, m]*(bm[-1 + n, -1 - m]*F[K, -2 + n, m, r, θ, ϕ] + bm[-1 + n, 1 + m]*F[K, -2 + n, 2 + m, r, θ, ϕ] + bp[-1 + n, -1 - m]*F[K, n, m, r, θ, ϕ] + bp[-1 + n, 1 + m]*F[K, n, 2 + m, r, θ, ϕ]) + bp[n, -m]*(bm[1 + n, 1 - m]*F[K, n, -2 + m, r, θ, ϕ] + bm[1 + n, -1 + m]*F[K, n, m, r, θ, ϕ] + bp[1 + n, 1 - m]*F[K, 2 + n, -2 + m, r, θ, ϕ] + bp[1 + n, -1 + m]*F[K, 2 + n, m, r, θ, ϕ]) + bp[n, m]*(bm[1 + n, -1 - m]*F[K, n, m, r, θ, ϕ] + bm[1 + n, 1 + m]*F[K, n, 2 + m, r, θ, ϕ] + bp[1 + n, -1 - m]*F[K, 2 + n, m, r, θ, ϕ] + bp[1 + n, 1 + m]*F[K, 2 + n, 2 + m, r, θ, ϕ]));

(* d_2, 3  <- archive definition 20, verified = D[.., x_2] D[.., x_3] *)
Subscript[d, 2, 3][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  (-(1/2))*I*K^2*(am[n, m]*(bm[-1 + n, -m]*F[K, -2 + n, -1 + m, r, θ, ϕ] + bm[-1 + n, m]*F[K, -2 + n, 1 + m, r, θ, ϕ] + bp[-1 + n, -m]*F[K, n, -1 + m, r, θ, ϕ] + bp[-1 + n, m]*F[K, n, 1 + m, r, θ, ϕ]) + ap[n, m]*(bm[1 + n, -m]*F[K, n, -1 + m, r, θ, ϕ] + bm[1 + n, m]*F[K, n, 1 + m, r, θ, ϕ] + bp[1 + n, -m]*F[K, 2 + n, -1 + m, r, θ, ϕ] + bp[1 + n, m]*F[K, 2 + n, 1 + m, r, θ, ϕ]));

(* d_3, 2  <- archive definition 21, verified = D[.., x_3] D[.., x_2] *)
Subscript[d, 3, 2][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Subscript[d, 2, 3][K, n, m, r, θ, ϕ][F];

(* d_3, 3  <- archive definition 22, verified = D[.., x_3] D[.., x_3] *)
Subscript[d, 3, 3][K_, n_, m_, r_, θ_, ϕ_][F_] :=
  Simplify[PowerExpand[K*(am[n, m]*(K*(am[n - 1, m]*F[K, n - 2, m, r, θ, ϕ] + ap[n - 1, m]*F[K, n, m, r, θ, ϕ])) + ap[n, m]*(K*(am[n + 1, m]*F[K, n, m, r, θ, ϕ] + ap[n + 1, m]*F[K, n + 2, m, r, θ, ϕ])))]];
