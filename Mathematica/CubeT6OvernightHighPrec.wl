...
(* Mp[p,q,r]: try symbolic Integrate[] first (returns exact closed
   form for all low-degree cases -- same approach CubeGalerkin27.wl
   uses successfully).  Fall back to NIntegrate only if Integrate
   fails or times out (120s budget per integral).                   *)
Mp[p_Integer, q_Integer, r_Integer] := Mp[p, q, r] = Module[
  {sym, real},
  Print["  Mp[", p, ",", q, ",", r, "] symbolic Integrate ..."];
  sym = TimeConstrained[
    Integrate[
  ...
  If[sym =!= $Failed && FreeQ[sym, Integrate],
    real = FullSimplify[Re[ComplexExpand[sym]]];
    Print["    -> exact closed form"];
    real,
    Print["    -> NIntegrate fallback at WP=", highWP];
    NIntegrate[
      x^p y^q z^r / Sqrt[x^2 + y^2 + z^2],
      {x, 0, 1}, {y, 0, 1}, {z, 0, 1},
