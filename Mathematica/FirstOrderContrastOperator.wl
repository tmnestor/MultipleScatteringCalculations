...
(* ============================================================================
   Part 9.  Export the two couplings, as a reference for the Python assembly.

   The single-site T-matrix is assembled in Python, from
   T = DeltaC (I - G DeltaC)^-1, where G is the self propagator moment.  Python
   rebuilds DeltaC_eff INDEPENDENTLY from DeltaA rather than importing it; this
   file is the arbiter that the two implementations agree, which is the
   evidence standard this project works to.  Two implementations agreeing is
...
exportCase[dl_, dm_, dr_] := Module[{rule},
   rule = {lamCst -> 17.5, muCst -> 22.5, rhoCst -> 2.5, w -> 60.0, aa -> 1.0,
     dlam -> dl, dmu -> dm, drho -> dr};
   <|"dlam" -> dl, "dmu" -> dm, "drho" -> dr,
     "dCeff_re" -> Re[N[dCeff /. rule]], "dCeff_im" -> Im[N[dCeff /. rule]],
     "dCnav_re" -> Re[N[dCnav /. rule]], "dCnav_im" -> Im[N[dCnav /. rule]]|>];

Export[refPath,
  <|"note" -> "auto-generated by FirstOrderContrastOperator.wl; do not edit",
    "param_order" -> {"c1", "c2", "c3", "e11", "e22", "e33", "e12", "e13", "e23"},
    "lam" -> 17.5, "mu" -> 22.5, "rho" -> 2.5, "omega" -> 60.0, "a" -> 1.0,
    "cases" -> {exportCase[0.0, 1.0, 0.0], exportCase[2.0, 1.0, 0.1],
      exportCase[20.0, 10.0, 0.5]}|>, "JSON"];

Print[""];
Print["--- Part 9: exported the coupling reference -------------------------"];
Print["  wrote ", refPath];
