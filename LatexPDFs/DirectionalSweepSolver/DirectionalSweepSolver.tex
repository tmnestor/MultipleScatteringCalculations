...
\midrule
amplitude $\times$ phase$^n$ reproduces the bundled kernel & post-$k_x$-residue kernel & $1.5\times10^{-15}$ \\
\quad and the residual is \emph{flat} in separation $n=1\ldots32$ & --- & --- \\
left-going amplitude is the $x$-reflection of the right & parity identity & $1\times10^{-15}$ \\
lateral sweep $=$ pairwise double sum, distinct sources & $O(N^2)$ sum & $1.5\times10^{-13}$ \\
\quad and the residual is \emph{flat} in lattice size & --- & --- \\
$k_y$-integrated kernel $=$ closed-form propagator & Kupradze closed form & falls to $2\times10^{-5}$ \\
inter-plane sweep $=$ exact real-space pairwise sum & Kupradze closed form & falls to $8\times10^{-4}$ \\
lateral coupling does not wrap (no periodicity) & kernel at true separation & $0$ \\
stratified operator reduces to whole space & independent $k_z$-residue kernel & $1.1\times10^{-15}$ \\
same-depth reverberation vanishes for a uniform background & --- & $<10^{-15}$ \\
self-energy $\equiv$ dressed T-matrix, $T_d=\Tz(I-D\Tz)^{-1}$ & direct vs dressed solve & $4\times10^{-16}$ \\
reverberation obeys symplectic reciprocity & $J_6$ law, $\pm k$ & $8\times10^{-14}$ \\
self-energy recedes with the reflector & --- & monotone, $10^{-4}\!\to\!10^{-12}$ \\
partition: every pair once, self-term per background & support count & exact \\
GMRES $=$ dense LU and Neumann series, $\rho(\Gz\Tz)=0.92$ & two direct methods & $1.2\times10^{-12}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\medskip\noindent
\textbf{Not yet done.} Two things. The in-out $k_y$ pair, and with it the full three-dimensional
Cartesian case, is the remaining piece; until it lands, the solver is 2\textonehalf-D and a
comparison against a three-dimensional solver measures the geometry rather than the sweeps.
And the \emph{absolute magnitude} of $\Delta\Gz$ is not yet checked against an independent
physical reference: what is established is that it is reciprocal, that it vanishes without
layering, that it recedes with the reflector, and that it behaves as a self-energy under the
dressed-T identity. The route to closing it is the single-bounce limit --- as the contrast
weakens, $\Delta\Gz$ must approach the interface reflection coefficient times the whole-space
propagator over twice the standoff --- which is checkable in isolation.
...
divergence cancels rather than as a term requiring regularisation. Second, the depth structure
